...
static int write_netlabel_rule(const char* srcdir) {
        ...
        if (netlabel_fd < 0) {
                if (errno != ENOENT)
                        log_warning("Failed to open '/sys/fs/smackfs/netlabel': %m");
        ...
        FOREACH_DIRENT(entry, dir, return 0) {
                ...
                FOREACH_LINE(buf, policy,
                        ...
                        if (write(netlabel_fd, buf, strlen(buf)) < 0) {
                                if (r == 0)
                                        r = -errno;
                                log_error("Failed to write '%s' to '/sys/fs/smackfs/netlabel' in '%s'", buf, entry->d_name);
                                break;
                        }
                }
        }
...
static int write_access2_rules(const char* srcdir) {
        ...
        if (change_fd < 0)  {
                if (errno != ENOENT)
                        log_warning("Failed to open '/sys/fs/smackfs/change-rule': %m");
                return -errno; /* negative error */
        }
        ...
        FOREACH_DIRENT(entry, dir, return 0) {
                ...
                /* load2 write rules in the kernel require a line buffered stream */
                FOREACH_LINE(buf, policy,
                             log_error("Failed to read line from '%s': %m", entry->d_name)) {
                        ...
                        if (sscanf(buf, "%ms %ms %ms %ms", &sbj, &obj, &acc1, &acc2) < 3) {
                                log_error("Failed to parse rule '%s' in '%s', ignoring.", buf, entry->d_name);
                                continue;
                        }
...
static int write_cipso2_rules(const char* srcdir) {
        ...
        if (cipso2_fd < 0)  {
                if (errno != ENOENT)
                        log_warning("Failed to open '/sys/fs/smackfs/cipso2': %m");
                return -errno; /* negative error */
        }
